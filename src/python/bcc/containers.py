...
def filter_by_containers(bpf_text, args):
    ...
    bpf_text = bpf_text.replace('CONTAINERS_FILTER_IMPL',
        """
                ...
                do {
                  ...
                  u64 ns_id = task->nsproxy->mnt_ns->ns.inum;
    ...
    if args.cgroupmap:
        bpf_text = bpf_text.replace('CGROUP_ID_SET', '1')
        bpf_text = bpf_text.replace('CGROUP_PATH', args.cgroupmap)
    else:
        bpf_text = bpf_text.replace('CGROUP_ID_SET', '0')

    if args.mntnsmap:
        bpf_text = bpf_text.replace('MOUNT_NS_SET', '1')
        bpf_text = bpf_text.replace('MOUNT_NS_PATH', args.mntnsmap)
    else:
        bpf_text = bpf_text.replace('MOUNT_NS_SET', '0')
